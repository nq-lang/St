...
struct IVSurface {
    ...
    std::vector<double> iv_grid;  // row-major [dte][moneyness] — double to match axis types for ImPlot3D::PlotSurface's single-template-type signature

    [[nodiscard]] double at(int dte_idx, int m_idx) const noexcept {
    ...
    data::IVSurface build_surface(bool is_call) {
        ...
        for (int dte_i = 0; dte_i < 30; ++dte_i) {
            ...
            for (int m_i = 0; m_i < 30; ++m_i) {
                ...
                surf.iv_grid[static_cast<size_t>(dte_i) * 30 + static_cast<size_t>(m_i)] = base_iv * 100.0;
            }
        }
        return surf;
    }
...
void plot_dual_bars(const char* id, const data::ExposureSeries& s, double spot, BarOrientation orient,
    ...
    if (ImPlot::BeginPlot(id, ImVec2(-1, -1), kPlotFlags)) {
        ...
        ImPlotBarsFlags bflags = orient == BarOrientation::Horizontal ? ImPlotBarsFlags_Horizontal : ImPlotBarsFlags_None;
        ImPlot::SetNextFillStyle(theme::rgb(call_color));
        ImPlot::PlotBars("Calls", s.strikes.data(), s.calls.data(), n, bar_w, bflags);
        ImPlot::SetNextFillStyle(theme::rgb(put_color));
        ImPlot::PlotBars("Puts", s.strikes.data(), s.puts.data(), n, bar_w, bflags);
        draw_spot_line(spot);
        ImPlot::EndPlot();
    }
}
...
void plot_signed_bars(const char* id, const std::vector<double>& strikes, const std::vector<double>& values,
    ...
    if (ImPlot::BeginPlot(id, ImVec2(-1, -1), kPlotFlags)) {
        ...
        const double bar_w = 0.8;
        ImPlotBarsFlags bflags = orient == BarOrientation::Horizontal ? ImPlotBarsFlags_Horizontal : ImPlotBarsFlags_None;
        if (!pos_x.empty()) { ImPlot::SetNextFillStyle(theme::rgb(theme::kEmerald)); ImPlot::PlotBars("Pos", pos_x.data(), pos_y.data(), (int)pos_x.size(), bar_w, bflags); }
        if (!neg_x.empty()) { ImPlot::SetNextFillStyle(theme::rgb(theme::kRed)); ImPlot::PlotBars("Neg", neg_x.data(), neg_y.data(), (int)neg_x.size(), bar_w, bflags); }
        draw_spot_line(spot);
        ImPlot::EndPlot();
    }
}
